...
#    pragma omp declare target(::pmacc::idDetail::nextId)
    ...
    typename IdProvider<T_dim>::State IdProvider<T_dim>::getState()
    {
        ...
        Environment<>::task(
            [](auto deviceData) { PMACC_KERNEL(idDetail::KernelGetNextId{})
                                  (1, 1)(deviceData.getDataBox()); },
            TaskProperties::Builder().label("KernelGetNextId").scheduling_tags({SCHED_CUPLA}),
        ...
        state.nextId = Environment<>::task(
                           [](auto hostData) { return static_cast<uint64_t>(hostData.getDataBox()(0)); },
                           nextIdBuf.host().data())
                           .get();
    ...
    {
        Environment<>::task(
            [nextId]() { PMACC_KERNEL(idDetail::KernelSetNextId{})
                         (1, 1)(nextId); },
            TaskProperties::Builder().label("KernelSetNextId").scheduling_tags({SCHED_CUPLA}));
    }
    ...
    uint64_t IdProvider<T_dim>::getNewIdHost()
    {
        ...
        Environment<>::task(
            ...
            TaskProperties::Builder().label("KernelGetNewId").scheduling_tags({SCHED_CUPLA}),
