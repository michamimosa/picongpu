...
namespace pmacc
{
    namespace mem
    {
         ...
         */
        template<typename T_Item, std::size_t T_dim, typename T_BorderItem = T_Item>
        struct GridBuffer : HostDeviceBuffer<T_Item, T_dim, grid_buffer::data::Access>
        {
             ...
             */
            GridBuffer(GridLayout<T_dim> const& gridLayout, bool sizeOnDevice = false)
                : HostDeviceBuffer<T_Item, T_dim, grid_buffer::data::Access>(gridLayout.getDataSpace(), sizeOnDevice)
            ...
            GridBuffer(DataSpace<T_dim> const& dataSpace, bool sizeOnDevice = false)
                : HostDeviceBuffer<T_Item, T_dim, grid_buffer::data::Access>(dataSpace, sizeOnDevice)
             ...
             */
            GridBuffer(
                buffer::GuardBase<device_buffer::DeviceBufferResource<T_Item, T_dim, grid_buffer::data::Access>> const&
                    otherDeviceBuffer,
                GridLayout<T_dim> const& gridLayout,
                bool sizeOnDevice = false)
                : HostDeviceBuffer<T_Item, T_dim, grid_buffer::data::Access>(
                    otherDeviceBuffer,
                ...
                bool sizeOnDeviceRecv = false)
            {
                if(hasOneExchange && (communicationTag != lastUsedCommunicationTag))
                    throw std::runtime_error(
                        "It is not allowed to give the same GridBuffer different communicationTags");
                    ...
                    if(send.isSet(ex))
                    {
                        ...
                        if(!sendExchanges[sendex])
                        {
                            sendExchanges[sendex].emplace(
                                get_device_exchange(sendex, dataPlace == GUARD ? BORDER : GUARD, guardingCells),
                                sendex,
                        ...
                        if(!recvExchanges[recvex])
                        {
                            recvExchanges[recvex].emplace(
                                get_device_exchange(recvex, dataPlace == GUARD ? GUARD : BORDER, guardingCells),
                                recvex,
                                uniqCommunicationTag,
                                useMpiDirect,
                                sizeOnDeviceRecv);
                ...
                if(dataSpace.productOfComponents() != 0)
                {
                    ...
                    for(uint32_t ex = 1; ex < 27; ++ex)
                    {
                        if(send.isSet(ex))
                        {
                            ...
                            {
                                sendExchanges[sendex].emplace(
                                    DeviceBuffer<T_BorderItem, T_dim, grid_buffer::data::Access>(
                                        dataSpace,
                                        sizeOnDeviceSend),
                                    sendex,
                                    uniqCommunicationTag,
            ...
            void addExchangeBuffer(
                const Mask& receive,
                const DataSpace<T_dim>& dataSpace,
                uint32_t communicationTag,
                bool sizeOnDevice = false)
            {
            ...
            auto get_device_exchange(ExchangeType exchangeType, AreaType dataPlace, DataSpace<T_dim> guardingCells)
            {
             ...
             */
            std::optional<Exchange> getSendExchange(uint32_t ex) const
            {
             ...
             */
            std::optional<Exchange> getReceiveExchange(uint32_t ex) const
            {
        ...
        private:
            ...
            std::array<std::optional<Exchange>, 27> sendExchanges, recvExchanges;
        };
...
template<typename T_Item, std::size_t T_dim, typename T_BorderItem>
struct redGrapes::trait::BuildProperties<pmacc::mem::GridBuffer<T_Item, T_dim, T_BorderItem>>
{
    template<typename Builder>
    static void build(Builder& builder, pmacc::mem::GridBuffer<T_Item, T_dim, T_BorderItem> const& buf)
    {
