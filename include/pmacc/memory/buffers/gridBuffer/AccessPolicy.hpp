    ...
    namespace mem
    {
                    ...
                    */
                            ...
                            rg::access::IOAccess::is_serial(a.mode, b.mode))
                        {
                            ...
                            else
                            {
                                for(int ex = 0; ex < 27; ++ex)
                                    if(a.direction.isSet(ex) && b.direction.isSet(ex))
                    ...
                    bool is_superset_of(Access const& other) const
                    {
                        if(
                            ...
                            (~this->area & other.area) == 0 && this->mode.is_superset_of(other.mode))
                        {
                            ...
                            else
                            {
                                for(uint32_t ex = 1; ex < 27; ++ex)
                                    if(!this->direction.isSet(ex) && other.direction.isSet(ex))
        ...
        struct buffer::data::ReadGuard<
            ...
            typename std::enable_if<
                std::is_same<typename Buffer::DataAccessPolicy, pmacc::mem::grid_buffer::data::Access>::value>::type>
            : buffer::data::ReadGuardBase<Buffer>
        {
            ...
            ReadGuard(
                ...
                : buffer::data::ReadGuardBase<Buffer>(base.sub_area(
                    exchange::exchangeTypeToOffset<Buffer::dim>(
                        exchangeType,
                        gridLayout,
                        guardingCells,
                        dataPlace == GUARD ? GUARD : BORDER),
                    exchange::exchangeTypeToDataSpace<Buffer::dim>(exchangeType, gridLayout, guardingCells)))
            ...
            {
                return std::vector<rg::ResourceAccess>({this->data.make_access(
                    grid_buffer::data::Access{rg::access::IOAccess::read, this->area, this->directions})});
            }
        ...
        public:
            ...
            WriteGuard(
                ...
                DataSpace<Buffer::dim> guardingCells,
                GridLayout<Buffer::dim> gridLayout)
                : buffer::data::WriteGuardBase<Buffer>(base.sub_area(
                    exchange::exchangeTypeToOffset<Buffer::dim>(
                        exchangeType,
                        gridLayout,
                        guardingCells,
            ...
            {
                return std::vector<rg::ResourceAccess>({this->data.make_access(
                    grid_buffer::data::Access{rg::access::IOAccess::write, this->area, this->directions})});
            }
        ...
        public:
            ReadGuard(GuardBase<Buffer> const& base) : buffer::GuardBase<Buffer>(base), area{CORE + BORDER + GUARD}
            {
                for(int i = 1; i < 27; ++i)
                    this->directions = this->directions + Mask(i);
            }
            ...
            ReadGuard(
                ...
                DataSpace<Buffer::dim> guardingCells,
                GridLayout<Buffer::dim> gridLayout)
                : buffer::GuardBase<Buffer>(base.sub_area(
                    exchange::exchangeTypeToOffset<Buffer::dim>(
                        exchangeType,
                        gridLayout,
                        guardingCells,
        ...
        public:
            ...
            WriteGuard(GuardBase<Buffer> const& base) : buffer::ReadGuard<Buffer>(base)
            {
            }
...
struct fmt::formatter<pmacc::mem::grid_buffer::data::Access>
{
    ...
    auto format(pmacc::mem::grid_buffer::data::Access const& a, FormatContext& ctx)
    {
