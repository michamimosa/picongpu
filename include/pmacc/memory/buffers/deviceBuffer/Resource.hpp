    ...
    namespace mem
    {
        namespace device_buffer
        {
            ...
            namespace data
            {
                ...
                template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy>
                struct ReadGuard
                    : pmacc::mem::buffer::data::ReadGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>
                {
                    cuplaPitchedPtr getCudaPitched() const
                    {
                        return this->data.obj->get_cuda_pitched();
                    }

                    ReadGuard<T_Item, T_dim, T_DataAccessPolicy> read() const noexcept
                    {
                        return *this;
                    }

                    ReadGuard(buffer::GuardBase<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>> const&
                                  base) noexcept
                    ...
                    ReadGuard(WriteGuard<T_Item, T_dim, T_DataAccessPolicy> const& wr) noexcept
                        : buffer::data::ReadGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>(
                            (buffer::data::WriteGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>> const&)
                                wr)
                    {
                    }
                };
                ...
                template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy>
                struct WriteGuard
                    : pmacc::mem::buffer::data::WriteGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>
                {
                    cuplaPitchedPtr getCudaPitched() const
                    {
                        return this->data.obj->get_cuda_pitched();
                    ...
                    ReadGuard<T_Item, T_dim, T_DataAccessPolicy> read() const noexcept
                    {
                        return ReadGuard<T_Item, T_dim, T_DataAccessPolicy>(*this);
                    }
                    WriteGuard<T_Item, T_dim, T_DataAccessPolicy> write() const noexcept
                    {
                        return *this;
                    }

                    WriteGuard(buffer::GuardBase<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>> const&
                                   base) noexcept
            ...
            namespace size
            {
                ...
                using ReadGuard = buffer::size::ReadGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>;

                template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy = rg::access::IOAccess>
                using WriteGuard = buffer::size::WriteGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>;
            } // namespace size


            template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy = rg::access::IOAccess>
            struct DeviceBufferResource
                : protected rg::SharedResourceObject<DeviceBufferData<T_Item, T_dim>, T_DataAccessPolicy>
            {
                using Item = T_Item;
                ...
                DeviceBufferResource(DataSpace<dim> capacity, bool use_vector_as_base = false)
                    : rg::SharedResourceObject<Data, DataAccessPolicy>(
                        std::make_shared<Data>(capacity, use_vector_as_base))
                ...
                DeviceBufferResource(
                    ...
                    : rg::SharedResourceObject<Data, DataAccessPolicy>(other.data)
                    , use_vector_as_base(use_vector_as_base)
                {
                }
            ...
            template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy = rg::access::IOAccess>
            using ReadGuard = buffer::ReadGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>;

            template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy = rg::access::IOAccess>
            using WriteGuard = buffer::WriteGuard<DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>;
        ...
        namespace buffer
        {
            ...
            {
                template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy>
                struct ReadGuard<device_buffer::DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>
                    : GuardBase<device_buffer::DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>>
                {
                    ReadGuard(
                        GuardBase<device_buffer::DeviceBufferResource<T_Item, T_dim, T_DataAccessPolicy>> const& base)
                    ...
                    DataSpace<T_dim> getCurrentDataSpace() const
                    {
...
template<typename T_Item, std::size_t T_dim, typename T_DataAccessPolicy>
struct redGrapes::trait::BuildProperties<pmacc::mem::device_buffer::data::ReadGuard<T_Item, T_dim, T_DataAccessPolicy>>
{
    template<typename Builder>
    static void build(
        Builder& builder,
