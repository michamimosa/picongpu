            ...
            public:
                ...
                BufferSize()
                {
                    ...
                    this->host_current_size
                        = rg::IOResource<size_t>(std::shared_ptr<size_t>(host_size_ptr, [](size_t* ptr) {
                ...
                virtual size_t get_current_size() const
                {
                    return Environment<>::task(
                               [](auto host_current_size) { return *host_current_size; },
                               TaskProperties::Builder().label("BufferSize::get_current_size()"),
                ...
                {
                    Environment<>::task(
                        [new_size](auto host_current_size) { *host_current_size = new_size; },
                        TaskProperties::Builder().label("BufferSize::set_current_size()"),
                ...
                DataSpace<dim> get_current_data_space() const
                {
                    ...
                    if(dim == DIM3)
                    {
                        ...
                        else
                        {
                            tmp[0] = data_space[0];
                            tmp[1] = data_space[1];
                            tmp[2] = (current_size + (data_space[0] * data_space[1]) - 1)
