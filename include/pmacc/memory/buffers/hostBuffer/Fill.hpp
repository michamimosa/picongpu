...
namespace pmacc
{
    namespace mem
    {
        namespace buffer
        {
            ...
            auto fill(host_buffer::WriteGuard<T_Item, T_dim, T_DataAccessPolicy> const& host_buffer, T_Item && value)
            {
                return Environment<>::task(
                    [value{std::forward<T_Item>(value)}](auto host_buffer) {
...
#pragma omp parallel for
                    ...
                    TaskProperties::Builder().label("fill HostBuffer"),
                    host_buffer.write());
            }
