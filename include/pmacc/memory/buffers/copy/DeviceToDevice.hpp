...
namespace pmacc
{
    namespace mem
    {
        namespace buffer
        {
            ...
            template<typename T, std::size_t T_Dim, typename T_DstDataAccessPolicy, typename T_SrcDataAccessPolicy>
            void copy(
                ...
                device_buffer::ReadGuard<T, T_Dim, T_SrcDataAccessPolicy> const& src)
            {
                Environment<>::task(
                    ...
                    TaskProperties::Builder().label("pmacc::mem::copy(dst: Device, src: Device)"),
                    dst.write(),
                    src.read());
            }
